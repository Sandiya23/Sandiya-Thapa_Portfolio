...
A single-page React site with a small `/sandmin` content studio. Content (projects,
...
The `/sandmin` page edits projects, experience, skills, and — on the **Contact** tab —
...
- **Local mode** — run `npm run dev` and open `http://localhost:8080/sandmin` (no login).
...
routes to `index.html` so client-side routes (e.g. `/sandmin`, `/work/:id`) resolve on
